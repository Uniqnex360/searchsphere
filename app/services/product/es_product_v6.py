...
async def get_product_list_v6(
    ...
    if parsed_filters.get("brand", []):
        if brand is None:
            brand = []
        brand.extend(parsed_filters.get("brand", []))

    if parsed_filters.get("category", []):
        if category is None:
            category = []
        category.extend(parsed_filters.get("category", []))
